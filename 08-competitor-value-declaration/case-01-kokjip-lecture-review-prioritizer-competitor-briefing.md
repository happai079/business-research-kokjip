...
> 다루는 것: 경쟁 사업자 8곳의 **규모·비즈니스 모델·전략 특징**, 핵심 가치와 직결된 사업 활동, 그 활동에서 **역산한 가치 선언문**, 그리고 그 여덟 개를 걸러 도출한 **콕집 자신의 가치 목표 3안**([§6-7](#6-7-참조-선언-필터링--여덟-개-중-무엇을-빌려-쓸-수-있는가)·[§6-8](#6-8-콕집이-추구할-차별적-가치-목표--세-방향)).
...
### 6-7. 참조 선언 필터링 — 여덟 개 중 무엇을 빌려 쓸 수 있는가

§5의 선언 여덟 개를 **콕집이 차용할 수 있는가**로 걸렀습니다. 판정 기준은 네 가지입니다.

| 기준 | 묻는 것 |
|---|---|
| ㉠ **판정을 말하는가** | 기록·보관이 아니라 **선별**을 선언하는가 |
| ㉡ **근거가 문장 안에 있는가** | 무엇을 기준으로 판정하는지 문장이 스스로 밝히는가 |
| ㉢ **희소 자원을 지목하는가** | 무엇이 부족하다고 말하는가 |
| ㉣ **3청중 안전** | 학습자·기관·강사 중 누구도 자극하지 않는가([§6-5](#6-5-그러나-뺄셈을-버릴-필요는-없다--뺄셈의-목적어를-바꾸면-된다)) |

| # | 선언 | ㉠ | ㉡ | ㉢ | ㉣ | 판정 |
|---|---|:-:|:-:|:-:|:-:|---|
| ③ | Gemini Notebook — "올린 자료 안에서만 대답한다" | △ | **◎** | ✕ | ○ | **전면 차용** |
| ⑤ | 티로 — "당신 목소리를 저장하지 않는다" | ○ | ○ | ✕ | **◎** | **전면 차용** |
| ④ | Otter.ai — "수업 시간에 받아적지 마세요" | **◎** | ✕ | ○ | △ | **조건부 차용**(반면교사 포함) |
| ⑥ | 노션 — "학생 땐 공짜, 취직하면 유료" | ✕ | ✕ | ○ | ○ | 부분 차용 — **시간축 화법만** |
| ⑧ | 패스트캠퍼스 — "무엇이 필요한지는 시장이 정한다" | ○ | ○ | ✕ | ✕ | 부분 차용 — **권위 위임 화법만** |
| ① | 다글로 — "말을 글로 바꾼다" | ✕ | ✕ | ✕ | ○ | **회피** — 기록 층. 무료 3곳과 정면충돌 |
| ② | 클로바노트 — "녹음은 공짜" | ✕ | ✕ | ✕ | ○ | **회피** — 카테고리 가격이 0임을 스스로 인정하는 문장 |
| ⑦ | 엘리스 — "강의도 하고 시스템도 판다" | ✕ | ✕ | ✕ | ✕ | **회피** — 서는 순간 교육 판매자가 되어 판정 중립성 상실 |

**차용 대상 셋에서 각각 가져올 것**

| 출처 | 빌려 쓰는 것 | 콕집에서의 형태 |
|---|---|---|
| **③ Gemini Notebook** | **근거의 범위를 선언에 박는 화법.** "자료 밖은 말하지 않는다"는 제약인데 신뢰로 읽힌다 | [§6-5](#6-5-그러나-뺄셈을-버릴-필요는-없다--뺄셈의-목적어를-바꾸면-된다)의 방어 논리("우선순위는 강사가 한 말에서 나온다")를 **선언으로 승격**시킬 문법 |
| **⑤ 티로** | **부정형 선언의 안전한 형태.** 여덟 곳 중 유일하게 "안 하는 것"을 선언하는데 아무도 불쾌하지 않다 — **부정의 목적어가 자기 자신**이기 때문 | 콕집의 위험한 뺄셈은 목적어가 **상대**였다("당신 강의에서 뺀다"). 같은 부정형인데 **방향이 반대** — 이 대조가 §6-5 수정의 핵심 근거 |
| **④ Otter.ai** | **행동을 없애 주겠다는 화법.** "받아적지 마세요"는 콕집의 "다 복습하지 마세요"와 문장 구조가 동일 | 단, 이 선언은 **녹음당하는 사람을 빠뜨려 소송으로 되돌아왔다**([§4-4](#4-4-otterai--이-시장을-처음-열었고-소송도-처음-맞았다)). **빠진 청중이 없는지 세는 절차까지가 이 참조의 내용** |

### 6-8. 콕집이 추구할 차별적 가치 목표 — 세 방향

§6-7의 차용 요소로 세 방향을 구성했습니다. **누구를 설득하는가**와 **언제 쓸 수 있는가**가 각각 다릅니다.

#### A안 「시간」 — 부족한 것은 강의가 아니라 당신의 시간이다

> **"복습은 다 하는 게 아니라, 남은 시간 안에 끝내는 겁니다.**
> **오늘 쓸 수 있는 시간을 알려주시면, 그 안에 들어갈 것만 골라 드립니다."**

**참조** ④ Otter(행동을 없앰) + ⑥ 노션(시간축). **뺄셈의 목적어를 「시간」으로 고정**해 [§6-4](#6-4-뺄셈-선언에는-치명적인-부작용이-있다--비어-있는-데는-이유가-있을-수-있다)의 두 반발(기관·강사)을 원천 차단합니다. 여덟 곳 중 **시간을 입력으로 받는 사업자는 0곳**이라 어필 강도가 가장 큽니다 🟢.

**약점** 근거가 없습니다. 시간에 맞춰 자르되 **무엇을 남길지의 기준이 문장 안에 없어** 단독으로는 임의 절삭으로 읽힙니다 🟡.

#### B안 「출처」 — 우선순위는 우리가 정하지 않는다, 강사가 정한다

> **"무엇이 중요한지는 우리가 판단하지 않습니다.**
> **강의 중에 강사가 '이건 실무에서 씁니다'라고 말한 그 순간을 찾아 되돌려 드릴 뿐입니다."**

**참조** ③ Gemini Notebook(근거 범위) + ⑤ 티로(부정형) + ⑧ 패스트캠퍼스(권위 위임). [딥리서치 §4-2](../04-tam-sam-som-market-segment-map/deep-research/kokjip-research.md#4-2-확정--강사-발화로-출발선을-취업-이력으로-벽을)가 확정한 **④ 강사 발화 1차**를 그대로 선언으로 만든 것이라, **세 청중 모두에게 안전한 유일한 안**입니다 — 강사에게는 권위의 증폭, 기관에게는 감사가 아님, 학습자에게는 판정 출처의 가시화.

**약점** ㉠ 차별화가 약해 보입니다("AI가 판단을 안 한다면 무엇이 좋은가") ㉡ **강사 발화 신호 밀도**([딥리서치 §7-1](../04-tam-sam-som-market-segment-map/deep-research/kokjip-research.md#7-미해소-항목))에 제품이 통째로 걸립니다 🔴.

#### C안 「결과」 — 먼저 합격한 사람이 실제로 붙잡았던 것

> **"이 과정을 먼저 마치고 취업한 사람들이 실제로 붙잡았던 것부터 보여드립니다.**
> **당신은 그 순서만 따라가면 됩니다."**

**참조** ⑥ 노션(시간이 지나야 쌓이는 자산). [딥리서치 §4-2](../04-tam-sam-som-market-segment-map/deep-research/kokjip-research.md#4-2-확정--강사-발화로-출발선을-취업-이력으로-벽을)가 **유일한 진짜 진입장벽**으로 지목한 ③ 취업 성공자 학습 이력을 선언으로 만든 것입니다. 기관의 통증(취업률 62.6% → 54.2% 🟢)에 직결돼 **지불자 설득력이 가장 큽니다.**

**약점** 시점입니다 — **첫 라벨이 9~12개월 뒤, 차수당 13명** 🟡. **오늘은 쓸 수 없는 문장**입니다.

#### 세 방향 비교와 권고

| | **A 「시간」** | **B 「출처」** | **C 「결과」** |
|---|---|---|---|
| 주 설득 대상 | 학습자(사용자) | 강사·기관(공급자·지불자) | 기관(지불자) |
| 오늘 쓸 수 있는가 | **○** | ○ | ✕ — 9~12개월 뒤 |
| 어필 강도 | **최상** | 중 | 상 |
| 3청중 안전성 | 상 | **최상** | 중 — 이해상충 소지 |
| 복제 난이도 | 낮음 | 중 | **최상** |
| 걸려 있는 미검증 | 시간 입력 UX의 실사용 🔴 | 강사 발화 신호 밀도 🔴 | 라벨 축적 속도·표본 🟡 |

> **권고 — 배타적 선택이 아니라 시간차 배치입니다.**
>
> **A를 겉면에, B를 같은 화면 안에 둡니다.** A 단독은 임의 절삭으로 보이고, B 단독은 *"그래서 무엇이 좋은가"* 에 답하지 못합니다. 둘을 붙이면 **"당신의 시간에 맞춰(A), 강사가 직접 말한 것부터(B)"** 가 되어 **어필과 안전이 동시에 성립합니다** 🟡. C는 12개월 뒤 B의 가중치를 교정하는 자리로 로드맵에 예약하고, 그 전까지는 라벨만 적립합니다([딥리서치 §4-2](../04-tam-sam-som-market-segment-map/deep-research/kokjip-research.md#4-2-확정--강사-발화로-출발선을-취업-이력으로-벽을)의 시점 설계와 동일).
>
> **⚠️ C에 대한 경고** — C를 앞당기려고 채용 연계를 붙이면 [딥리서치 §3-3](../04-tam-sam-som-market-segment-map/deep-research/kokjip-research.md#3-3-다만-이-값에는-세-개의-함정이-있습니다)의 이해상충이 그대로 발동합니다. 우리가 우선순위를 정하고 그 결과로 우리가 매칭하면 **매칭 성사를 위해 우선순위를 왜곡할 유인**이 생깁니다. **C는 "보여주는 것"까지이며, "연결하는 것"으로 넘어가는 순간 A·B의 신뢰가 함께 무너집니다.**
...
| 8 | **A안의 「남은 시간 입력」을 학습자가 실제로 쓰는가** 🔴 — [§6-8 A안](#a안-시간--부족한-것은-강의가-아니라-당신의-시간이다)의 전제. 입력 자체가 마찰이면 선언은 성립해도 제품이 안 돌아간다 | 프로토타입 5~10명 사용성 테스트 · 기본값(예: 30분) 대비 입력률 측정 |
| 9 | 엘리스 2024 실적(351억/134억)의 **감사보고서 원문** 🟡 → 🟢 승격 | DART 조회 |
...
| **가치 선언 3안과 배치 순서** — A(시간) 겉면 + B(출처) 동일 화면, C(결과)는 12개월 뒤 예약([§6-8](#6-8-콕집이-추구할-차별적-가치-목표--세-방향)) 🟡 | [Ch05 여정지도](../05-persona-spectrum-journey-map/case-01-kokjip-lecture-review-prioritizer-journey-map.md) 첫 접점 메시지 · [Ch06 전략](../06-market-opportunity-score/case-01-kokjip-lecture-review-prioritizer-strategy.md) 착수 순서에 반영 |
| **C안(결과)은 「보여주기」까지** — 채용 연계로 넘어가면 이해상충이 A·B의 신뢰까지 무너뜨린다 🟡 | [딥리서치 §3-3](../04-tam-sam-som-market-segment-map/deep-research/kokjip-research.md#3-3-다만-이-값에는-세-개의-함정이-있습니다) 이해상충 항목에 **선언 차원의 귀결** 추가 |
