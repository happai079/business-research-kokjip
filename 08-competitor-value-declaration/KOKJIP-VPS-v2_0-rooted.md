...
2. **가치 A (시간 예산 절약 · Delivery Framing) — 전달 형식 (종속 요소)**
   - 학습자의 핵심 Pain("남은 시간에 무엇을 버릴지 결정")에 접근하는 UI 인터페이스이자 진입 메시징이다.
...
3. **가치 C (취업/시험 결과 연계 · Outcome Evidence) — 현 단계 VP에서 전면 제외**
   - 9~12개월의 시차, 극소 표본 문제(기수당 수료생 수 제한), 채용 시장 진입 시 발생하는 중립성 훼손 리스크로 인해 현재 VP에서 제외하고 데이터 로그 적립 대상으로만 관리한다.
...
**주 표적 (Core Persona):** 고밀도 직무형 부트캠프(K-Digital Training 등 AI·SW·데이터 과정 3~6개월 차)에 참여 중인 비전공 취업준비생

| 타깃 구분 조건 | 선정 및 제외 이유 | 근거 등급 |
|---|---|:---:|
| **일 6~8시간 고밀도 연강 수강** | 매일 대량의 녹음·코드가 누적되어 복습 우선순위 선별 Pain이 집중되는 집단 | 🟢 사실 (KDT 표준 편성) |
| **직무형 교육 (개발/데이터)** | 이론과 실무의 괴리가 커 현직자 강사의 실무 팁이 높은 정보 가치를 지닌 분야 | 🟡 추론 (산업 구조 분석) |
| **자격증/공시생 제외 (배제 집단)** | 정답과 합격선이 정형화되어 강사의 주관적 실무 강조 신호가 작동하지 않음 | 🟡 추론 (시험 구조 분석) |
| **장기 미취업 청년 제외 (배제 집단)** | 태깅할 원천 강의 데이터가 없어 제품의 동작 전제가 성립하지 않음 | 🟢 사실 (제품 작동 조건) |
...
| CJM 단계 | 고객이 겪는 실제 Pain | 고객의 본질적 Needs (Desired Outcome) | 대체재 한계 |
|---|---|---|---|
| **1. 수업 직후** | 하루 6~8시간 분량의 음성/자료 누적으로 압도감 발생 | 오늘 반드시 복습해야 할 핵심 분량만 1차 선별 | 단순 전사는 분량 압축 불가 |
| **2. 복습 계획** | 일반 AI 요약은 교재 수준 이론만 나열하고 실무 팁을 누락 | "강사가 실무·면접에서 쓰인다고 한 내용" 중심 필터링 | 범용 LLM은 실무 맥락 부재 |
| **3. 복습 실행** | "강사가 중요하다고 했던 구간"을 찾으려 배속 재청취로 시간 낭비 | 실무 강조 발화 지점으로 1클릭 즉시 이동 | 타임스탬프 없는 텍스트 요약의 한계 |
| **4. 맥락 검증** | AI 텍스트 요약만 보면 코드 작성 맥락이나 강사 뉘앙스 오해 | 요약 텍스트와 원천 오디오의 1:1 동기화 검증 | LLM 환각(Hallucination) 불안 |

### 2-3. 핵심 Pain의 구조 (PP-3)

> **핵심 Pain (PP-3): "남은 시간 안에 무엇을 버리고 무엇을 먼저 볼지 결정하기 어렵다."**  
> *(근거: 고객 여정 분석 모델 상 기회점수 1순위 도출 Pain 🟡)*
...
- **궁극적 Desired Outcome:** 제한된 훈련 기간(3~6개월) 내에 기업 채용 기준에 부합하는 실무 역량을 확보하는 것.
...
|---|---|:---:|---|
| 1 | 학습자는 남은 시간에 무엇을 버릴지 결정하고 싶어 한다 | 🟡 **추론** | 고객 여정 분석 모델 상 결핍 점수 1위 도출 |
...
| **프로젝트 시점 복습 내용 망각** | 프로젝트 구현 중 과거 팁 즉시 호출 | 맥락 재소환 | **프로젝트 키워드 기반 강의 구간 역검색** | **P1 (차기)** |
| **강의 자료 유출 및 저작권 우려** | 강사/기관의 안심 도입 | Data Containment (신뢰 기본선) | **음성 전사 후 원본 즉시 폐기 & 학습 미사용** | **P0 (핵심 필수)** |
...
### 9-1. 무료 경쟁 회피와 과금 접점 가설

- **무료 경쟁 구간 회피:** 클로바노트·노션 등 무료 조합에 만족하는 일상 강의 청취 시점(탐색 단계)에서는 마케팅 비용을 투입하지 않는다.
- **과금 접점 가설:** 배운 내용을 실제 프로젝트에 적용해야 하지만 과거 강의 위치를 찾지 못해 병목을 겪는 시점에 유료 전환 가치가 발생할 것으로 가설을 수립한다.

### 9-2. B2B 기관 도입의 역할

- **배포 관문으로서의 기관:** 수강생 개인의 자율 설치에 의존하기보다, KDT 훈련기관과의 제휴를 통해 입과 시점에 솔루션을 일괄 배포하는 방식을 핵심 유통 가설로 설정한다.
- **기관 소구 가치:** 단순 "수강생용 복습 도구"가 아닌 "수강생 학업 이탈 방지 및 취업 성과 관리" 관점의 가치를 제공하는 방향으로 포지셔닝을 검토한다.

### 9-3. 필수 신뢰 및 데이터 보호 원칙

- **강사 저작권 보호:** 강의 녹화물 및 실연권에 대한 강사의 권리를 존중하며, 강사의 거부 권리를 보장하는 운영 원칙을 수립한다.
