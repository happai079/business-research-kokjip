...
| [세그먼트 지도](#세그먼트-지도--정제-결과를-한-장으로) | 정제 결과를 3장의 차트로 — 2×2 좌표 · 탈락 깔때기 · 경쟁 밀도 |
...
조사한 12개 사업자 중 해당 기능을 **공개 자료에서 확인할 수 있었던 곳의 수**입니다. 이 카운트가 [지도 3](#지도-3--기능별-경쟁-밀도-히스토그램)의 근거입니다.

| 기능 | 확인 수 | 선점 사업자 | 상태 |
|---|---|---|---|
| 직무·역량 진단 | **8** | 멀티캠퍼스(KAIST 8대 역량·40항목), 넥스트젠(L1~L5), 팀스파르타, 원티드, 상상력집단, axlab, 한국AX교육연구원, KOSA | **레드오션** |
| 자기 업무 과제 기반 실습 | **7** | 멀티캠퍼스(해커톤), 팀스파르타, 원티드(4~9주), 상상력집단, axlab, 한국AX교육연구원(MVP), KOSA(PBL) | **레드오션 + 무료 공급 존재** |
| 에이전트·워크플로 구축 | **7** | 원티드(Ennoia), 팀스파르타, 멀티캠퍼스, SK AX, JYT AI, 상상력집단, 한국AX교육연구원 | **레드오션** |
| AI 활용률 계측 | **4** | 넥스트젠(AI-Q·AI Gain), axlab(8%→67% 판매 언어화), 팀스파르타(98% 주장), 상상력집단 | **레드오션** |
| 성과 측정·ROI 리포트 | **3** | 상상력집단, axlab, 넥스트젠 | 경쟁 존재 |
| **직무 단위 도메인 깊이**(제조 품질검사·금융 규제문서 등) | **1** | 한국AX교육연구원이 제조·창업 특화 주장(레퍼런스 미공개) | 부분 미선점 |
| **종료 후 유지율(90일) 계측** | **0** | 확인된 사업자 없음 | **미선점** |
| **성과 연동 과금(risk-sharing)** | **0** | 확인된 사업자 없음 | **미선점** |
| **고객사 귀속 사례 라이브러리** | **0** | 원티드 Ennoia는 자사 플랫폼 락인 | **미선점** |
...
### 세그먼트 지도 — 정제 결과를 한 장으로

정제 1~6은 각각 다른 축으로 시장을 쪼갰습니다. 그 결과를 읽는 사람이 한 번에 이해하려면 **어떤 시각화 형식이 이 데이터의 성격에 맞는지**부터 골라야 합니다.
...
| **깔때기형 분기도** | 정제 1→2→6이 **순차적 탈락 구조**다. 구매주체 → 예산항목 → 기업규모 → 경쟁상태 순으로 후보가 걸러지고 마지막에 하나만 남는다. 왜 하나만 남는지를 추적하게 해준다 | **채택 — 논리 추적** |
| **히스토그램 (막대)** | 기능별 경쟁 사업자 수는 **단일 값의 순위 비교**다. 막대가 0에 닿는 항목이 곧 빈 자리이므로 공백이 시각적으로 부정할 수 없게 된다 | **채택 — 실행** |
| 산점도·좌표평면 | 2×2 매트릭스가 사분면 라벨까지 붙은 상위 호환 | 매트릭스로 대체 |
| **벤다이어그램** | 세그먼트가 **상호배타적**이어서 교집합이 거의 없다. 겹치는 것은 세그먼트가 아니라 사업자의 영업 범위이며, 그것은 이미 5-6의 수렴 그래프로 표현됨 | 부적합 |
| **트리맵·파이** | 세그먼트별 **절대 규모 비교**에 최적인 형식이지만, 국내 AI 교육 시장의 절대 규모는 이 리서치의 미확정 항목이다([한계 1](#이-리서치의-한계)). 없는 값을 면적으로 그리면 가장 설득력 있게 거짓말하는 차트가 된다 | 부적합 |
| **산키 (흐름도)** | 예산이 교육비·툴·구축으로 갈라지는 흐름 표현에 최적. 그런데 **그 배분 비율이 바로 부록 B-1의 1순위 미해소 항목**이다. 값이 확정되면 그때 산키가 정답이 된다 | 보류 — 데이터 확보 후 |
| **히트맵** | 사업자 × 기능 격자의 밀도 표현에 적합하나, Mermaid에 네이티브 히트맵이 없고 같은 정보를 히스토그램이 더 단순하게 전달한다 | 히스토그램으로 대체 |
...
**읽는 법** — 다섯 점이 네 사분면에 흩어진 것이 이 리서치의 핵심입니다. 하나의 시장으로 보였던 것이 실제로는 처분이 정반대인 구역들의 집합이었습니다.
...
| 현업·DX 1000인 미만 | 예산은 증가하나 **KOSA 무료 공급이 가격 상한을 0으로** | 표적에서 제외 |
| **현업·DX 1000인 이상** | 예산 증가 + 자산 축적 가능 | **유일한 유료 표적.** 단 12개 사업자가 이미 진입 |

**우상단이 비어 있지 않다는 것이 초판과 개정판의 차이입니다.** 초판은 이 칸을 빈 땅으로 봤고, 5-6의 실사는 여기가 가장 붐비는 칸임을 보여줍니다.
...
flowchart TD
    ...
    Q2 -->|"AI 도입·운영비 · 현업·DX"| Q3{"③ 기업 규모<br/>정제 6"}
    Q3 -->|"1000인 미만"| X3["탈락<br/>KOSA 등 무료 공급 · 가격 상한 0"]
    Q3 -->|"1000인 이상"| Q4{"④ 경쟁 상태<br/>정제 5"}
...
**이 그림의 요점은 탈락 사유가 전부 다르다는 것입니다.** B2C는 수요가 없어서, HRD는 예산이 없어서, 1,000인 미만은 무료 경쟁 때문에, 그리고 마지막 분기는 **경쟁자가 이미 있어서** 탈락합니다. 마지막 하나만 남는 것이 아니라, **남은 하나 안에서도 대부분이 이미 점유돼 있다**는 것이 개정판의 결론입니다.

#### 지도 3 — 기능별 경쟁 밀도 (히스토그램)

지도 2의 마지막 분기를 확대한 것입니다. 조사한 12개 사업자 중 각 기능을 제공하는 곳의 수이며, 카운트 근거는 [5-6의 기능별 선점 현황](#5-6-h5-검증--빈-땅이-아니었다)에 있습니다.
...
**세 개의 0이 이 리서치가 도달한 최종 답입니다.** 왼쪽 다섯 개(8·7·7·4·3)는 이제 차별화가 아니라 통과 요건입니다. 여섯 번째인 단일 직무 깊이는 1이지만 그 한 곳도 레퍼런스를 공개하지 않았으므로 사실상 공백에 가깝습니다. 그리고 오른쪽 세 항목을 아무도 하지 않는 이유는 어렵기 때문이 아니라 **단기 매출에 불리하기 때문**입니다(정제 5). 신규 진입자가 자원 열위를 뒤집을 수 있는 자리는 통상 이런 형태로만 존재합니다.

**세 지도는 같은 시장을 배율만 달리해 본 것입니다.** 지도 1은 어디에 앉을지(좌표), 지도 2는 왜 거기만 남는지(과정), 지도 3은 그 안에서 무엇을 할지(빈칸)를 답합니다. 셋을 순서대로 읽으면 7단계 솔루션의 기능 목록이 그대로 도출됩니다.
