...
분리 문서 — **[세그먼트 지도](./segment-map.md)**(정제 결과 시각화 3장) · **[SRS](./srs-ax-adoption-platform.md)**(7단계 제품 명세)
...
같은 주제·같은 방법론으로 수행된 선행 리서치 2건 — [Gemini 판](./리서치%20원문/AI_AX_EdTech_Research_Report_Gemini.md) · [GPT 판](./리서치%20원문/AI_AX_교육_에듀테크_산업_리서치_GPT.md)

**세 리서치가 독립적으로 같은 결론에 도달했습니다** — 범용 AI 툴 강의의 가치 붕괴, 미해결 과업은 학습이 아니라 정착, 해법은 진단→업무 과제→성과 측정→확산 파이프라인, 방어력은 도메인·프로세스 결합에서 나온다.

다만 5-6의 실사는 이 수렴에 다른 해석을 붙입니다. **세 AI가 같은 답에 도달한 이유는 그 답이 옳기 때문일 수도 있고, 그것이 이미 시장의 공통 언어이기 때문일 수도 있습니다.** 실제로 12개 사업자가 같은 구성으로 영업하고 있었으므로 **수렴은 정답의 증거가 아니라 포화의 증거일 수 있습니다.**
...
| 에이전트 구축 | 두 문서 모두 핵심 모듈로 포함 | **의도적 제외** — 이해상충 |
| B2G | Gemini는 성장 타깃에 포함 | 성장하나 전환비용이 안 생김. 현금흐름원과 자산 축적처를 분리 |
| 선투자 | 플랫폼·샌드박스 선구축 전제 | 자산화 선투자는 **이익을 먼저 깎는다**(엘리스 감가상각 52→126억). 컨시어지 선행 |

### 선행 리서치의 단서를 추적한 결과

초판은 두 선행 문서를 대조 목적으로만 인용했고, GPT 판이 참고 사례로 남긴 국내 사업자 두 곳(axlab, 한국AX교육연구원)은 확인하지 않았습니다. **그 두 링크를 실제로 열어본 것이 이 리서치의 결론을 바꿨습니다** — 12개 사업자 실사(5-6) → H5 기각 → 정제 4·5 신설.

GPT 판 15장의 "국내외 AX 교육 서비스 10~15개를 13개 항목으로 비교" 과제는 부록 B의 벤치마크 틀로 채택했습니다.

**교훈**: 초판은 "미탐색 영역"이라는 판정을 **검증 없이 전제로 깔았고**, 반증 단서가 이미 저장소 안에 있었습니다. 다음 리서치에서는 **"이 시장에 이미 누가 있는가"를 3단계의 필수 항목**으로 고정합니다(방법론에 반영).

**본문 논증에 쓰지 않았으나 기록하는 선행 자료** — 국내 에듀테크 시장 2022년 약 7.9조 → 2026년 약 10.8조(HolonIQ 인용, K-12 포함이라 범위 밖) · WEF Future of Jobs 2025의 스킬 39% 변화·스킬격차 장애물 63%·업스킬링 계획 77%. WEF 수치는 3-3의 "전문성 부족 45%"와 같은 방향이며 수요 측 진단이 **글로벌 공통 현상**임을 보여줍니다. 반대로 **지불 측 제약(예산 동결·단가 상한)은 국내 실측치에서만 확인**됐으므로 해외 확장 시 재검증이 필요합니다.

---

## 부록 B. 다음 리서치로 넘기는 질문

**미해소 항목** (솔루션 성패에 영향이 큰 순서)

- [ ] **기업 AI 예산의 교육/툴/구축 배분 비율** — 정제 2의 핵심 주장이 이 값 위에 있다. PoC 3개사 표본으로는 부족
- [ ] **1,000인 이상 국내 기업의 실제 수** — 정제 5가 표적을 이 구간으로 한정했으므로 **TAM의 분모 그 자체**
- [ ] **경쟁사의 비공개 계약 형태** — 유지율 계측·성과 연동이 실제로 미선점인지(한계 7)
...
**경쟁사 벤치마크 조사 설계** — GPT 판의 13개 항목(대상·가격·교육방식·기간·커리큘럼 단계·업무진단·실습·프로젝트·자동화·사후관리·성과측정·조직확산·B2B 모델)을 채택했고 5-6에서 부분 실행했습니다. 미확인으로 남은 **최우선 3개**가 정제 4의 차별화 축을 확정하거나 기각합니다.

| 최우선 확인 항목 | 왜 |
|---|---|
| **사후관리 기간** | 미확인. 종료 후 책임 범위가 곧 차별화 축 |
| **성과측정 방식·산식** | 주장만 확인, 산식 미공개 |
| **B2B 모델의 성과 연동 존재 여부** | 미확인. 있으면 차별화 축 하나가 사라진다 |

**Ch04로 넘기는 것** — 정제 5가 SAM 경계를 1,000인 이상으로 정했고, 5-2의 계정당 상한(2,500만~5,000만 원)이 SOM 계산의 단가입니다. 위 첫 두 항목이 확정되면 **SOM이 추정이 아니라 산술**이 됩니다.
