...
| 단계 | 상태 | 산출물 |
|---|---|---|
| **① Pain 리스트 정리** | **본 문서** | **대표 Pain 17건 + Goal + 트랙 분리** |
| ②~⑤ Importance · Satisfaction · AOS · Matrix | 완료 | [사례 01-②③④⑤ AOS Matrix](./case-01-kokjip-lecture-review-prioritizer-aos-matrix.md) |

[CJM]의 Pain Point **25건**(PP-1~8 · PP-K1~K8 · PP-A1~A3 · PP-E1~E3 · PP-N1~N3)에서 **페르소나 5명 × 3건 = 15건**을 대표로 추리고, **[스펙트럼]의 오세연(A2) 카드에서 2건을 추가 승격**해 **총 17건**을 다룹니다.

> **PP-U1·PP-U2는 이 문서에서 새로 부여한 ID입니다.** [CJM]은 [Ch05 방법론]의 **「주인공 1명 고정」** 원칙에 따라 Adjacent 유형의 주인공을 한도윤으로 두었고, 같은 유형의 **오세연(대학 4학년)은 §5의 분기 산문**으로만 처리해 PP ID를 받지 못했습니다. 그런데 대학(L3)은 [Ch04] SAM 모집단의 **62~77%** 를 차지하는 최대 집단이라, ID 없이 두면 **SAM 다수의 Pain이 채점에서 통째로 빠집니다.**
>
> **접두어를 `PP-A*`가 아니라 `PP-U*`(University)로 둔 이유** — `PP-A1~A3`은 이미 한도윤 전용입니다. 같은 Adjacent 유형이지만 **지불 구조도 여정 시작점도 달라**(한도윤은 자비 결제로 시작, 오세연은 학기 단위로 반복) 같은 계열 번호를 쓰면 채점 척도가 섞입니다.

---

## 1. 선별 기준 — 25건을 15건으로 줄인 방법 (+ 후속 승격 2건)
...
| **오세연** · Adjacent(대학) | **"학점을 지키면서 취업에 쓰일 것만 골라 익히고 싶다"** | 카드 목표란 *"전공 학습 중 취업에 쓰일 부분만 골라내기. 학점을 지키며 시간 확보"* | 돕는다 |
...
> **오세연만 Goal이 둘이고 서로 경합합니다.** 나머지 다섯은 목표가 하나(합격 · 취업률 · 회수 증명 · 준법 학습 · 자산 통제)인데, 오세연은 **학점**과 **취업 준비**를 동시에 지켜야 합니다. **이 경합 자체가 아래 PP-U2의 정체**이며, 다른 페르소나에는 대응물이 없습니다.
...
| **PP-U1** | 오세연 · 학기 중 상시 | **"흩어진 전공 강의를 한 곳에 모아 전체를 보고 싶다"** | 강의가 **학기·과목 단위로 분산돼 총량이 파악되지 않는다** | 과목별 자료 개별 보관 · **노션 학생 무료** 🟢 수기 정리 · 무료 녹음 앱 | 🔴 | 순위 |
| **PP-U2** | 오세연 · 학기 중 상시 | **"학점을 지키면서 취업 준비 시간도 확보하고 싶다"** | **학점과 취업 준비가 같은 시간을 두고 경합한다** | **한쪽을 포기하기** — 스펙 위주 준비(학점 포기) 또는 학점 우선(취업 미루기) | 🔴 | 순위 |
...
| 트랙 | 건수 | 처리 방식 |
|---|---|---|
| **순위** | **11건** | PP-3 · PP-4 · PP-6 · PP-K1 · PP-K7 · PP-A1 · PP-A2 · PP-A3 · **PP-U1 · PP-U2** · PP-E2 → **AOS 채점 후 내림차순 정렬** |
...
**첫째, Goal은 Pain의 부정문이 아닙니다.** PP-3의 Goal을 "판단 기준을 갖고 싶다"로 쓰면 그것은 목표가 아니라 **우리 제품의 기능 설명**입니다. 고객은 판단 기준을 원한 적이 없습니다 — [CJM §3-1]에서 김지원이 실제로 한 말은 *"이걸 다 볼 수는 없는데"* 이고, 원하는 것은 **버릴 것을 정하는 일**입니다. 이 구분이 중요한 이유는 Importance 채점에서 갈리기 때문입니다. Goal을 기능으로 쓰면 "그 기능이 얼마나 중요한가"를 묻게 되고, 그것은 **우리가 만들고 싶은 것의 중요도**이지 고객의 중요도가 아닙니다. 열일곱 건 모두 **제품 이름과 기능 명칭이 들어가지 않는 문장**으로 썼습니다.
...
> **요약하면, Goal 열은 세 가지를 걸러냅니다** — ① 기능을 목표로 착각한 항목 ② 주어가 고객이 아닌 항목 ③ Goal 달성이 Pain을 키우는 항목. 열일곱 건 중 **여섯 건(PP-E3 · PP-N3 · PP-A2 · PP-4 · PP-U1 · 문지훈 3건 전체)이 여기에 걸렸고**, 걸린 항목은 전부 **점수를 매기기 전에 취급 방식을 먼저 정해야 하는 것들**이었습니다.
...
| PP-7 (포트폴리오가 똑같다) | 1 | 확장 기능 영역. 🔴 가설이고 기존 생태계 Pain이 아니다 |
...
| PP-K4 (예산 항목 불명확) 🔴 | 실질적 조달 장애물이지만 **🔴 가설.** ②단계 이전에 사실 확인이 먼저 |
...
### 4-6. 오세연 (Adjacent · 대학) — 카드 3건 → 2건 승격 *(후속 추가)*

**[CJM]이 아니라 [스펙트럼] 카드에서 직접 가져온 유일한 항목**이라 다른 다섯과 절차가 다릅니다. 카드의 「문제」란에 세 가지가 적혀 있고, 그중 둘을 승격했습니다.

| 카드의 문제 | 처분 | 이유 |
|---|---|---|
| a. 전공 강의가 **취업에 실제로 쓰이는지 판단이 안 선다** | **승격 안 함** | **PP-3과 같은 축**이다. 단위(밀린 강의 ↔ 전공 과목)와 계기(복습 부채 ↔ 학기)만 다르고, 핵심은 둘 다 **"현직자 기준의 중요도 판단이 없다"**. 중복 채점을 피하고 **PP-3의 MR에 반영**한다 |
| b. 강의 밀도는 낮으나 **분산돼 관리가 더 어렵다** | **→ PP-U1** | 다른 페르소나에 **대응물이 없다.** 부트캠프는 집중 과정이라 분산되지 않는다 |
| c. **학점과 취업 준비가 경합** | **→ PP-U2** | 대학에만 있는 제약. **§2-1에서 오세연만 Goal이 둘인 이유** |

**둘 다 순위 트랙입니다.** 게이트가 아닌 이유는 이 Pain들이 조달을 막지 않기 때문입니다 — 대학은 [Ch04 §2-4]에서 지불 주체로 **보류**라 애초에 조달 경로가 없고, 오세연은 개인 구독자로만 존재합니다.

> **여정 단계를 「학기 중 상시」로 적은 것은 여정지도가 없기 때문입니다.** [CJM §5]는 오세연에 대해 *"①②가 없다(결제·본전 의식 없음). 학기 단위로 ③이 반복된다"* 고만 말합니다. **단계를 특정하려면 대학 CJM을 따로 그려야 하며, 그 전까지 이 두 건의 여정 위치는 미확정입니다.**
...
| 넘기는 것 | 내용 |
|---|---|
| **채점 대상** | **순위 트랙 11건** (PP-3 · PP-4 · PP-6 · PP-K1 · PP-K7 · PP-A1 · PP-A2 · PP-A3 · **PP-U1 · PP-U2** · PP-E2) |
...
| 검증 질문 | 결과 |
|---|---|
| 각 Pain이 **특정 페르소나에 귀속**돼 있는가 | ○ 17건 전부. 집단 평균 Pain 없음 |
