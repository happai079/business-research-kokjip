...
**채점 대상은 [①]의 순위 트랙 9건**입니다. 게이트 6건은 점수를 매기지 않고 §6에서 통과 요건으로 관리합니다.
...
### 1-5. 인식 격차 — 이 표에서 먼저 읽어야 할 것

**아홉 건 중 넷에서 고객의 인식과 실제 기여도가 갈립니다.** 격차의 **부호**가 처방을 바꿉니다.
...
**격차가 0인 다섯 건(PP-3 · PP-6 · PP-K1 · PP-K7 · PP-E2)이 「기능으로 푸는」 항목입니다.** 나머지 넷은 점수가 어떻게 나오든 **제품 기능의 문제가 아닙니다.**
...
| **6** | PP-K7 | 박성호 | 3 | 3 | 0.4 | **1.20** | 3 × (1 − 0.6) |
| **6** | PP-A3 | 한도윤 | 3 | 3 | 0.4 | **1.20** | 3 × (1 − 0.6) |
| **8** | PP-4 | 김지원 | 2 | 4 | 0.2 | **0.40** | 2 × (1 − 0.8) |
| **8** | PP-A2 | 한도윤 | 2 | 4 | 0.2 | **0.40** | 2 × (1 − 0.8) |
...
**1~5 정수 척도에서 동점은 필연입니다.** 아홉 건 중 네 쌍이 동점이므로 **순위 숫자가 아니라 군집 사이의 간격**을 읽습니다.

| 군집 | AOS | 항목 | 간격 |
|---|---|---|---|
| **상위 5건** | 3.20 ~ 2.40 | PP-A1 · PP-E2 · PP-3 · PP-K1 · PP-6 | ↓ **1.20** ← 최대 간격 |
| **중위 2건** | 1.20 | PP-K7 · PP-A3 | ↓ 0.80 |
| **하위 2건** | 0.40 | PP-4 · PP-A2 | — |

**상위 5건과 중위 2건 사이의 1.20이 이 목록에서 유일하게 신뢰할 만한 경계입니다.** 상위 5건 안의 3.20 대 3.00(0.20 차이)은 [방법론 §「경고」]대로 **채점자의 기분 차이와 구분되지 않으므로 순위로 읽지 않습니다.**
...
**경계는 각 지표의 중앙값**을 씁니다. 채점 대상 **9건**으로 계산하며, 재채점 항목 PP-3′는 같은 Pain의 중복이므로 **중앙값 계산에서 제외**하고 참고 점으로만 표시합니다.

| 지표 | 정렬된 값 (n=9) | **중앙값 (5번째)** |
|---|---|:---:|
| **Importance** | 2, 2, 3, 3, **4**, 4, 4, 5, 5 | **4** |
| **Satisfaction** | 1, 1, 2, 2, **2**, 3, 3, 4, 4 | **2** |
...
| 💎 **Q2 개선기회** | I ≥ 4 **&** S ≥ 2 | **PP-3** · **PP-K1** · **PP-6** | 3.00 · 3.00 · 2.40 | 지속적 개선 필요 |
| ⚫ **Q3 유지관리** | I < 4 **&** S < 2 | **없음** | — | — |
| ⚠️ **Q4 과잉투자** | I < 4 **&** S ≥ 2 | PP-K7 · PP-A3 · **PP-4** · **PP-A2** | 1.20 · 1.20 · 0.40 · 0.40 | 자원 분배 재검토 |
...
**1~5 정수 척도에서 중앙값은 반드시 실제 데이터 값과 겹칩니다.** 아홉 건 중 **여섯 건이 어느 한 축의 경계선 위에** 있습니다.
...
**아홉 건 중 다섯 건이 사분면을 갈아탑니다.** 그런데 **AOS 값과 순위는 하나도 바뀌지 않습니다** — 경계는 AOS 계산에 들어가지 않기 때문입니다.
...
**① 상위 5건과 나머지 4건 사이가 유일하게 신뢰할 만한 경계입니다.** AOS 2.40과 1.20 사이의 1.20이 최대 간격이고, 그 위의 다섯 건이 실행 후보입니다. 상위 5건 안의 0.20 차이는 순위로 읽지 않습니다.
...
**④ AOS 1위 PP-A1은 기능으로 풀 항목이 아닙니다.** §1-5에서 **격차 −2(과대 인식)** 로 표시된 항목입니다. 고객은 결제 직후 불안을 크게 느끼지만 **불안 해소가 실제 투자 회수를 만들지는 않습니다.** 처방은 개발이 아니라 **온보딩 메시지**이고, 근거 등급도 🔴 순수 가설입니다. **AOS 1위에 개발 자원을 태우면 안 되는 사례**이며, 인식 격차 열이 없었다면 그대로 최우선 개발 항목이 됐을 것입니다.
...
| 처분 | 항목 | 근거 |
|---|---|---|
| **기능으로 개발** | **PP-3 · PP-6 · PP-K1** | 상위 군집 · **인식 격차 0** · Q1/Q2 |
| **조건부 개발** | **PP-E2** | AOS 공동 1위 + 게이트 해제 효과. **단 시장 크기 확인이 선행** |
| **메시지·프레이밍으로** | **PP-4 · PP-A2 · PP-A1** | 인식 격차 ±2 이상. **점수와 무관하게 기능이 답이 아님** |
...
| Satisfaction이 **대체 솔루션의 만족도**인가 | ○ §2에 9건 전부 비교 대상 명시. 「없음」 2건도 명시 |
| Importance와 Satisfaction을 **분리해서** 매겼는가 | ○ §1과 §2를 다른 패스로 수행(채점 규칙 2) |
| 각 Pain이 **특정 페르소나에 귀속**됐는가 | ○ §1을 페르소나별 표 4개로 분리 |
