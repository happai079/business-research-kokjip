...
| 포함 | **학습자를 겨냥해 설계된** 강의 복습·학습 우선순위 도구 |
| 제외 | 강의 콘텐츠 자체(교육 공급), LMS(기관 운영 시스템), K-12 학습지, 어학 앱, **범용 녹음·요약·문서 도구** |
| **산업 내부 플레이어** | **다글로**(*"대학생 맞춤형 AI"* 표방 🟢) 및 학습 특화 복습 도구 소수 |
| **인접 산업 = 대체재** | 클로바노트·에이닷 노트·티로(범용 녹음·요약) · 노션(범용 정리) · AI 코스웨어(기관 판매) → [§2-4](#2-4-대체재의-위협--매우-강-55) |
...
**분류 규칙 — 같은 압박을 두 번 세지 않기 위해 못박습니다.** 포터의 5가지 힘에서 **경쟁자와 대체재는 상호 배타**입니다. 그래서 **"학습자를 겨냥해 만들어졌는가"** 하나로 갈랐습니다.

| 판정 | 대상 | 어느 힘으로 세는가 |
|---|---|---|
| **산업 내부(경쟁자)** | 학습자용으로 설계된 도구 — **다글로** | [§2-5 경쟁 강도](#2-5-산업-내-경쟁-강도--중-35) |
| **인접 산업(대체재)** | 학습자를 겨냥하지 않았지만 같은 욕구를 채우는 것 — 클로바노트·노션·AI 코스웨어 | [§2-4 대체재](#2-4-대체재의-위협--매우-강-55) |

클로바노트는 **회의록 도구**이고 노션은 **범용 문서 도구**입니다. 학생이 쓴다는 사실이 그것들을 이 산업의 사업자로 만들지는 않습니다 — **다른 산업에서 같은 욕구를 채우는 것**, 즉 대체재의 정의 그대로입니다. 따라서 **무료 티어가 가격을 0으로 누르는 압력은 §2-4에서만 계산하고 §2-5에서는 다시 세지 않습니다.**
...
|---|---|---|---|---|
| 1 | 공급자 교섭력 | 강 | 4 | **원재료(강의 발화)의 접근권을 강사가 거부할 수 있고**, 모델 제공사는 같은 기능을 무료로 직접 판다 |
...
| 5 | 기존 경쟁 강도 | 중 | 3 | 좁힌 자리는 아직 비어 있으나, **다글로가 같은 산업 안에서 한 걸음 거리**에 있고 차별화가 어렵다 |
...
> 한 줄 결론: **강.** 단 **4점을 지탱하는 것은 API 원가가 아니라 접근권과 무료 전방 통합**입니다. 모델 종속은 위에서 스스로 반증했듯 설계로 해소되므로 점수 근거에서 뺐습니다. 남는 것은 돈으로 풀리지 않는 두 가지입니다 — **강사는 거부할 수 있고, 공급자는 같은 기능을 공짜로 판다.**
...
- **전환비용을 쌓을 시간 자체가 없습니다.** 일반 SaaS는 쓰다 보면 데이터가 축적돼 이탈이 어려워지는데, 이 제품은 그 축적이 일어나기 전에 **강의가 끝나 수요 자체가 소멸합니다.** 고객이 떠나는 것이 아니라 **고객이기를 그만두는** 구조라, 전환비용이 낮은 것이 아니라 **걸 대상이 사라집니다.**
...
**이 5점은 다글로 때문이 아닙니다.** 다글로는 신규 진입자가 아니라 **이미 이 산업 안에 있는 경쟁자**이므로, 그들이 우리 자리로 넘어오는 위협은 여기가 아니라 [§2-5](#2-5-산업-내-경쟁-강도--중-35)에서 셉니다(§0 분류 규칙).

이 항목이 5점인 이유는 **자본도 자격도 없이 누구나 같은 제품을 만들 수 있다**는 것 하나입니다. 콕집이 오늘 이 문으로 들어가는 것과 정확히 같은 방식으로 내년에 다른 팀이 들어옵니다. **진입장벽의 부재는 우리에게만 열린 문이 아닙니다.**
...
**아래는 전부 이 산업 밖에 있습니다**(§0 분류 규칙). 학습자를 겨냥해 만들어지지 않았지만 같은 욕구를 채우므로 대체재이고, **이들이 가격을 0으로 누르는 압력은 이 항목에서만 계산합니다.**
...
**다만 대체재 전부가 못 하는 자리가 하나 있습니다.** 위 항목의 우선순위는 **학습자 내부 데이터**(내가 틀린 것)이거나 **강의와 무관한 일반론**(로드맵)입니다. **"이번 주 이 강의에서 나온 이 개념"** 수준의 해상도로 실무 중요도를 판정하는 사업자는 **경쟁 실사 5종에서 확인되지 않았습니다** 🟡(미발견이지 부재의 증명은 아닙니다). 비전공 전향자는 **자기가 무엇을 모르는지조차 모르므로** 오답 기반 우선순위가 작동하지 않는다는 것이 이 빈자리의 근거입니다.
...
**이 항목은 산업 내부 사업자끼리의 출혈만 셉니다.** 무료 티어 압력은 §2-4에서 이미 계산했으므로 여기서 다시 세지 않습니다.

**압박을 낮추는 쪽**

- **좁힌 자리가 비어 있습니다.** "국비 직업교육 학습자 × 실무 기준 우선순위"를 하는 사업자를 **경쟁 실사 5종에서 찾지 못했습니다** 🟡 — 데스크 조사 범위 안에서의 미발견이지 부재의 증명은 아닙니다.
- **시장이 커지는 국면입니다.** KDT 예산이 3,068억(2022) → **4,731억(2024)** 으로 늘었고 🟢, 성장기에는 서로의 몫을 뺏지 않아도 성장할 수 있어 출혈이 늦게 옵니다.
- 산업 내부에 **학습 특화 사업자 자체가 적습니다.**

**압박을 높이는 쪽**

- **다글로가 한 걸음 거리입니다.** 2017년 출시된 국내 최초 STT 사업자로 **"대학생 맞춤형 AI"를 명시적으로 표방**하며 강의 녹음 → 요약 → **AI 퀴즈 생성**까지 이미 제공합니다 🟢. 콕집이 더하려는 것은 "현직자 기준 중요도 판정" 한 겹인데, **기술이 아니라 판단 근거 데이터의 문제**라 프롬프트만 바꿔 따라올 수 있습니다 🔴.
- **차별화가 어렵고 전환비용이 없습니다.** 학습자는 앱을 바꾸는 데 아무 비용도 치르지 않습니다.
- **성과 입증 국면입니다.** 에듀테크는 혁신이라는 이름만으로 확산되지 않고 **실제 학습 성과를 입증해야 살아남는 단계**로 진입했습니다 🟢. 기능 경쟁이 곧 증빙 경쟁으로 바뀝니다.

> 한 줄 결론: **중(3).** 다섯 힘 중 유일하게 압박이 낮지만, 그 이유는 콕집이 강해서가 아니라 **아직 아무도 이 좁은 자리에 안 왔기 때문**입니다. 그리고 그 자리에서 가장 가까운 곳에 **이미 다글로가 서 있습니다.** 이 3점은 자산이 아니라 **유효기간이 있는 기회**이며, 다글로가 실무 중요도 축을 붙이는 순간 4~5로 올라갑니다.
...
```mermaid
graph TD
    B["공급자 교섭력<br/>강 (4)<br/>강사 거부권 + 무료 전방통합"] --> A
    C["구매자 교섭력<br/>매우 강 (5)<br/>무료 대체재 + 기관 178곳"] --> A
    D["신규 진입자 위협<br/>매우 강 (5)<br/>자본·자격 없이 누구나"] --> A
    E["대체재 위협<br/>매우 강 (5)<br/>최대 대체재 = 안 하기"] --> A
    F["기존 경쟁 강도<br/>중 (3)<br/>다글로가 한 걸음 거리"] --> A
...
> **경계 판정이 점수 구성을 바꿉니다 — 총점보다 이쪽이 중요합니다.** 클로바노트·노션을 산업 내부로 넣으면 경쟁 강도가 올라가고 대체재가 내려가, **같은 22점이라도 처방이 정반대로 나옵니다**(내부로 보면 "경쟁사를 이겨라", 외부로 보면 "무료가 못 하는 일을 찾아라"). §0에서 **"학습자를 겨냥해 만들어졌는가"** 로 경계를 고정한 것은 이 처방을 가르기 위해서입니다. 이 분석은 후자를 채택했고, 그래서 [§4](#4-전략적-시사점)의 결론이 기능 경쟁이 아니라 **판정 근거와 지불 주체**로 갑니다.
