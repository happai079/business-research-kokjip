# 사례 02 — 「콕집」의 TAM-SAM-SOM 과 세그먼트 맵

분석 기준 시점: 2026년 8월 · 적용 방법론: [Ch04 방법론](./methodology.md)

> **이 문서가 Ch04의 주 사례입니다.** [사례 01](./case-01-ai-education-edtech.md)은 이전 서비스 정의(B2B AX 교육 플랫폼) 기준으로 작성됐고, 분석 대상이 바뀌었으므로 참고용으로만 남겨 둡니다.

근거 문서
- [Ch01 5가지 힘 — AX/DX 교육 산업](../01-porters-five-forces/case-01-ai-education-edtech.md) — 이하 **[1A]**
- [딥 리서치 본문](./deep-research/case-ai-ax-edtech.md) — **[R]** (공급자 산업 분석. 이 서비스는 **학습자 측 도구**이므로 일부만 승계)
- 원천 데이터·계산 감사 → **[deep-research/kokjip-sizing-evidence.md](./deep-research/kokjip-sizing-evidence.md)**
...
| 축 | 사례 01 (B2B AX 플랫폼) | **사례 02 (콕집)** |
|---|---|---|
| 고객 | 기업 HRD·현업 부서장 | **학습자 개인 + 교육기관** |
| 구매 단위 | 계정당 연 8,000만 원 | **월 5,000원 구독 또는 기관 라이선스** |
| 파는 것 | AI 정착률 계측 | **복습 우선순위 판단** |
| 산업 층위 | 교육 **공급자** 측 | 교육 **수요자(학습자)** 측 |

**[R]의 딥 리서치는 공급자 산업을 분석한 것이므로 대부분 승계되지 않습니다.** 다만 세 가지는 그대로 유효합니다 — ① 정부 훈련 예산 구조 ② 무료 공급이 가격 상한을 누른다는 구조 ③ 부트캠프 시장의 취업률 압박.
...
| # | 항목 | 현재 처리 | 영향 |
|---|---|---|---|
| **1** | **개인 유료 전환율 3%** | 🔴 가정 | SOM의 62%가 여기 걸려 있다. 1%면 SOM 5.6억, 5%면 13.5억 |
| **2** | **기관당 연 라이선스 단가 1,000만 원** | 🔴 가정 | 경로 B에 선형 영향. 기관 지불 의사 실측이 없다 |
...
| 5 | 모집단 중복 제외 후 330만 명 | 🔴 추정 | 대학생·취준생·훈련생이 겹친다 |
| 6 | 1인당 연 지불액 6만 원 | 🔴 가정 | 유사 도구가 무료라 실측 근거가 약하다 |
| 7 | 직무형 교육 비중 40% · 복습 부채 필터 25% | 🔴 가정 | SAM에 선형 영향 |
...
**② 그래서 핵심 가치를 우선순위 판단에 둔 것은 옳습니다.** 무료 대체재가 못 하는 것은 **"현직자라면 무엇을 먼저 볼까"** 라는 판단 하나입니다. 이 판단의 품질이 곧 유료 근거이고, 여기가 약하면 제품 전체가 무료 도구와 구별되지 않습니다.
...
**④ 확장 기능(포트폴리오·프로젝트 매칭)은 확장이 아니라 본 수익원일 수 있습니다.** 학습 보조 구독의 단가 상한은 월 5,000원 수준으로 고정돼 있습니다. 반면 **채용 연계는 단가 구조가 완전히 다른 시장**입니다. 스크린샷은 이것을 "확장(유료)"으로 뒤에 뒀지만, 산정 결과는 **여기가 규모의 유일한 돌파구**임을 시사합니다. 다만 채용 시장 규모는 이번에 조사하지 않았으므로 [§8](#8-다음-챕터와-다음-리서치로-넘기는-것)의 1순위 리서치 항목으로 둡니다.
...
**다음 리서치 1순위** — **채용·구인 연계 시장의 단가 구조.** §7 ④의 결론이 이 값 위에 서 있고, 이번 산정에서 확인하지 않았습니다. 확인되면 SOM의 상한 자체가 바뀝니다.
...
| SAM 필터에 통과율과 근거가 붙었는가 | ○ 2개 필터. 단 둘 다 🔴 가정임을 표기 |
...
| 추정치와 실측치가 구분되는가 | ○ 🟢🟡🔴 표기 + §6에 가정 7건 전량 명시 |
