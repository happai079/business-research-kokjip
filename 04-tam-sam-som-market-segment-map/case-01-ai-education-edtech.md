...
| **SOM** | **약 8억 원/년** (3년차, 기본안) | 단일 도메인(금융 규제·심사 문서) × 계정 10개 × 계정당 8,000만 원 |
...
flowchart TD
    ...
    S -->|"⑤ 단일 도메인 1개 · 금융·보험 10%"| D["68억<br/>도메인 내 시장"]
    D -->|"⑥ 3년 내 획득 계정 10개"| O["<b>SOM 8억</b><br/>기본안"]
...
**표적 도메인은 금융·보험의 규제·심사 문서 업무**로 확정했습니다 — 은행 여신심사 의견서, 준법 검토, 상품 심사 등. 후보 셋(제조 품질검사 / 금융 규제문서 / 병원 행정) 중 선택이며, 선택 근거와 SOM에 미치는 영향은 [§5-4](#5-4-도메인을-금융으로-확정한-근거와-대가)에 있습니다.

| 단계 | 값 | 근거 |
|---|---|---|
| 1,000인 이상 기업 | 약 2,000개(가정) | §3-3 |
| × 도메인 필터 (**금융·보험 가정 10%**) | **약 200개** | 단일 도메인 진입([3] KSF1). **🔴 미확정 — §6 2순위** |
| × S4 세그먼트 비중 (도입했으나 일부 부서만) | 57% → **약 114개** | [R] 3-3 (도입률 76% × 일부 부서 75%) |
| × 3년 내 도달·전환 가능 | 약 9% → **약 10개** | 컨시어지 운영 한계(사당 3개월) + B2B 영업 사이클. 모집단이 작아져 **침투율로 환산하면 8.8%** |
...
| 시나리오 | 계정 수 | 계정당 | **SOM(3년차)** | 도메인 내 시장(68억) 대비 | SAM(680억) 대비 |
|---|---|---|---|---|---|
| 보수 | 6개 | 6,000만 원 | **3.6억 원** | 5.3% | 0.5% |
| **기본** | **10개** | **8,000만 원** | **8.0억 원** | **11.8%** | **1.2%** |
| 공격 | 15개 | 1억 원 | **15.0억 원** | 22.1% | 2.2% |

**기본안 8억은 [R] 5-2가 계산한 "연 매출 30억을 만들려면 대기업 계정 약 86개"의 4분의 1 규모입니다.** 단일 도메인에 머무는 한 15억이 상한에 가깝고, **두 번째 도메인을 여는 시점이 곧 성장 한계에 닿는 시점**입니다.

### 5-4. 도메인을 금융으로 확정한 근거와 대가

**근거 — 세 후보 중 금융이 표적 세그먼트와 가장 잘 맞습니다.**

| 조건 | 금융·보험(규제·심사 문서) | 제조(품질검사) | 병원(행정) |
|---|---|---|---|
| 1,000인 이상 기업 다수 | ○ 은행·보험사·카드사 | ○ | △ 상급종합병원 한정 |
| **S4 세그먼트 부합**(도입했으나 일부 부서만) | ○ **도입률 70%인데 규제로 확산이 막힘** 🟢 | ○ 도입률 44% — 도입 자체가 미완인 곳이 많음 | △ 데이터 확보 어려움 |
| 도메인 지식이 진입장벽 | ○ 규제 준수 문서·심사 의견서는 외부인이 못 만든다 | ○ | ○ |
| 거부권 보유자(준법·보안)의 존재 | ○ **가장 강하게 작동** — 개입 1순위의 설득력이 여기서 최대 | △ | ○ |
...
|---|---|---|
| 도메인 필터 | 25% 🔴 | **10%** 🔴 |
| 도메인 내 1,000인 이상 기업 | 약 500개 | **약 200개** |
| 도메인 내 시장 | 170억 원 | **68억 원** |
| SOM(기본안) | 15.2억 원 | **8.0억 원** |

금융·보험은 제조보다 **종사자 모집단이 작습니다.** 대규모 조직 편중도가 높아 1,000인 이상 구간에서는 전체 고용 비중보다 높게 나타나지만, 제조에는 못 미칩니다. 즉 **표적 적합도를 얻는 대가로 시장 크기를 절반 내주는 거래**입니다.

**이 거래를 받아들인 이유**는 [3] KSF1이 요구한 것이 "큰 도메인"이 아니라 "**신규 진입자가 비대칭을 가질 수 있는 도메인**"이기 때문입니다. 8억이라는 SOM은 3년차 목표로 작고, 그래서 [§7 ④](#7-전략적-함의)의 성장 경로(두 번째 도메인·단가 인상·해외)를 Ch06에서 반드시 비교해야 합니다.
...
**② 그래서 협소화가 손해가 아닙니다.** SAM 680억에서 단일 도메인(금융 규제·심사 문서)으로 좁히면 68억이 됩니다. 넓게 가도 680억이 상한이고, 넓게 가면 12개 사업자와 정면으로 부딪칩니다([R] 5-6). **좁히는 비용이 낮고 넓히는 이익이 작다** — 이것이 [3] KSF1을 금액으로 정당화합니다.
...
**④ 성장 경로는 도메인 확장뿐입니다.** 단일 도메인 SOM 상한이 약 15억이므로, 그 이상은 ① 두 번째 도메인 ② 계정당 단가 인상(성과 연동) ③ 해외 중 하나입니다. 세 경로 모두 Ch06 기회점수에서 비교 대상이 됩니다.
...
| SAM 680억 · 도메인 내 68억 | **Ch06 기회점수** | 시장 크기 항의 입력 |
