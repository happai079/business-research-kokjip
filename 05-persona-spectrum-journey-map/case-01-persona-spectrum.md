...
### A2. 정혜원 — 교수 저항에 막힌 대학 교수학습개발센터 팀장
...
### E2. 서말순 — 학습 자체가 위협인 25년차 시니어 실무자
...
### N1. 임규호 — 생성 AI를 금지한 금융사 준법감시인 *(못 쓴다)*
...
### N2. 남기석 — 자체 교육으로 해결하겠다는 사내 AI TF 리더 *(안 쓴다)*
...
⑤단계에서 완성할 관계 구조의 초안입니다. **선의 방향은 "확산·전환의 가능 경로"** 입니다.

```mermaid
graph LR
    subgraph CORE["Core — S4 세그먼트 내부"]
        C1["C1 김도현<br/>생산기술팀장<br/>예산 소유"]
        C2["C2 박수진<br/>HRD 파트장<br/>게이트키퍼"]
        C3["C3 이정민<br/>DX 팀장<br/>보안 게이트"]
        C4["C4 최은영<br/>품질검사 담당<br/>성패 결정"]
        C5["C5 한지훈<br/>AI 챔피언<br/>확산 채널"]
    end

    subgraph ADJ["Adjacent — 같은 문제, 다른 제도"]
        A1["A1 오세라<br/>공공기관 교육담당"]
        A2["A2 정혜원<br/>대학 CTL 팀장"]
        A3["A3 유상호<br/>로펌 KM 팀장"]
    end

    subgraph EXT["Extreme — 최다 실패"]
        E1["E1 강병철<br/>폐쇄망 3교대"]
        E2["E2 서말순<br/>25년차 시니어"]
    end

    subgraph NON["Non-user"]
        N1["N1 임규호<br/>준법감시인<br/>못 쓴다"]
        N2["N2 남기석<br/>사내 TF 리더<br/>안 쓴다"]
    end
    ...
    C1 -->|"성공 사례 이식"| A3
    C2 --> A2
    N2 -.->|"자체 제작 실패 시<br/>전환 가능"| C2

    style CORE fill:#e6f6ff,stroke:#33BBFF
    style ADJ fill:#e6fff6,stroke:#33FFBB
    style EXT fill:#ffece9,stroke:#FF3322
    style NON fill:#f2f2f2,stroke:#999
```

**이 그림에서 읽히는 것 (①단계 잠정 관찰)**

| 관찰 | 함의 |
|---|---|
| **N1이 Core 전체를 끊을 수 있다** | 비활성 사용자가 단순한 미고객이 아니라 **거부권 보유자**다. 보안 요구사항은 기능이 아니라 **진입 조건** |
| **C5 → C4가 유일한 수평 확산 경로** | 공식 교육(C2 발주)과 별개로 동료 전파가 실제로 일어나고 있다. 이 경로를 제품이 흡수하면 확산 비용이 내려간다 |
| **C4의 실패가 E1·E2로 흘러간다** | 표적 도메인(제조) 안에 계측이 성립하지 않는 인구가 있다. **SOM 산정의 계정당 대상 인원이 과대일 수 있다** |
| **N2 → C2 전환선이 유일한 보류 세그먼트 진입로** | 가격이 아니라 "자체 제작보다 빠르다"가 전환 논거 |
