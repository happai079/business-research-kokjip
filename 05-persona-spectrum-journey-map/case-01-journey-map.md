...
graph TB
    ...
    subgraph EXT["Extreme · 윤태경(녹음 금지반) — 3단계에서 끊긴다"]
        direction LR
        e1["① 과정 입과"] --> e2["② 녹음 금지 공지"] --> e3["③ 필기 시도 → 강의 놓침"] --> e4(("여정 종료<br/>입력 미발생"))
    end

    subgraph NON["Non-user · 문지훈(녹음 금지 강사) — 도입에 도달하지 않는다"]
        direction LR
...
| **② 녹음 금지 공지** | 윤태경 | 여정이 여기서 끊긴다. **이 단계의 존재 자체가 발견** 🔴 |
...
| **⑤ 재진입 조건** | 자산 귀속·삭제가 보장되면 재검토 | "내가 태깅 데이터의 주인이 된다면" | 조건부 개방 | **기회** — **녹음물·파생 데이터의 통제권과 삭제 시점을 강사·기관에 두는 설계.** 유출 우려를 없애 거부권을 해제한다 |
...
| **AI의 「현직자 관점」이 무엇을 근거로 하는지** | 근거가 정해지지 않으면 **다글로가 프롬프트만 바꿔 따라온다** 🟢 |
...
|---|---|---|---|
| **1** | **강사 자산 귀속·삭제 보장을 도입 안내에 선제시** (PP-N1·PP-K5) | 강사 ② = 기관 운영자 ⑤ = 학습자 ② | **거부권이 만들어지는 순간에 개입한다.** 막히면 차수 전체가 사라진다. 녹음물·파생 데이터의 통제권과 삭제 시점을 강사·기관에 두면 유출 우려가 사라져 거부권이 해제된다 |
...
| **2** | **AI의 「현직자 관점」의 근거 데이터가 무엇인가** 🔴 | **진입장벽의 존재 여부.** 후보는 채용공고의 기술 수요·현직자 커뮤니티·취업 성공자 학습 이력·강의 중 강사 발화. 정해지지 않으면 다글로와 구별되지 않는다([R] §6 1순위) |
