...
**코드가 아니라 프로필로** 부릅니다. 표적 계정은 **KDT 훈련기관 한 곳의 한 차수(수강생 약 25명)** 이고, 열둘 중 여덟 명이 이 차수 안에 있습니다.

### 주인공 네 명 (유형별 1명)

| 유형 | 호칭 | 프로필 |
|---|---|---|
| **Core** | **전향 수강생** | KDT 데이터·개발 과정 수강생(28세). 문과 전공 후 전향, 하루 8시간·6개월 과정. 「뭘 먼저 봐야 할지 모르겠다」 |
| **Adjacent** | **자비 수강생** | 사설 유료 부트캠프 수강생(29세). 수강료를 본인이 부담. 「돈 낸 만큼 뽑아야 한다」 |
| **Extreme** | **녹음 금지반 수강생** | KDT 수강생(27세). 강사·기관이 강의 녹음을 금지한 과정에 배정됨. 「녹음 금지라고 하셨다」 |
| **Non-user** | **녹음을 금지한 강사** | KDT 과정 강사. 강의 자료 유출 우려로 녹음을 정책으로 금지. 「내 강의가 밖으로 나간다」 |

### 여정에 함께 등장하는 여덟 명

| 호칭 | 프로필 | 이 여정에서의 위치 |
|---|---|---|
| **기관 운영자** | KDT 훈련기관 운영 책임자(45세). 전국 178곳 중 하나 | **기관 지불자.** 별도 구매 여정을 가짐([§4](#4-지불자-여정--기관-운영자)) |
| **현직자 강사** | KDT 과정 강사(38세). 현직 실무자 | **진입장벽 + 거부권.** 태깅 판단의 공급원 |
| **상위권 수강생** | 같은 차수 스터디 리더(26세) | 비공식 확산 채널 |
| **취업지원 담당자** | 같은 기관 취업지원 담당(34세) | 확장 기능의 사내 파트너 |
| **알바 병행 수강생** | 같은 차수 수강생(30세). 강의 후 야간 알바 | Extreme 분기 — 시간이 0 |
| **대학 4학년** | 졸업 학년(24세). 전공과 취준 병행 | Adjacent 분기 — 부채 중간 |
| **전환 준비 직장인** | 재직 중 직무 전환 준비(33세) | Adjacent 분기 — 녹화 강의 시청 |
| **무료 조합 사용자** | 클로바노트+노션+ChatGPT를 이미 쓰는 수강생 | Non-user 분기 — 안 쓴다 |
...
```mermaid
graph TB
    subgraph CORE["Core · 전향 수강생 — 취업까지 간다"]
        direction LR
        c1["① 과정 입과"] --> c2["② 진도 추격"] --> c3["③ 복습 부채 자각"] --> c4["④ 도구 탐색"] --> c5["⑤ 우선순위 복습"] --> c6["⑥ 프로젝트 수행"] --> c7["⑦ 수료·포트폴리오"] --> c8["⑧ 면접·취업"]
    end

    subgraph ADJ["Adjacent · 자비 수강생 — 투자 회수 확인으로 끝난다"]
        direction LR
        a1["① 자비 결제 결정"] --> a2["② 본전 의식"] --> a3["③ 복습 부채 자각"] --> a4["④ 유료 도구 즉시 결제"] --> a5["⑤ 복습·프로젝트"] --> a6["⑥ 취업"] --> a7["⑦ 환급 조건 정산"]
    end

    subgraph EXT["Extreme · 녹음 금지반 수강생 — 3단계에서 끊긴다"]
        direction LR
        e1["① 과정 입과"] --> e2["② 녹음 금지 공지"] --> e3["③ 필기 시도 → 강의 놓침"] --> e4(("여정 종료<br/>입력 미발생"))
    end

    subgraph NON["Non-user · 녹음을 금지한 강사 — 도입에 도달하지 않는다"]
        direction LR
        n1["① 도입 안내 수신"] --> n2["② 자료 유출 가능성 검토"] --> n3["③ 녹음 금지 결정"] --> n4["④ 차수 전체 차단"] --> n5(("재진입 조건<br/>자산 귀속·삭제 보장"))
    end
...
|---|---|---|
| **③ 복습 부채 자각** | 전향 수강생 | **제품의 진입점.** 입과 직후엔 문제가 없고, 3주차쯤 부채가 임계를 넘으면서 판단 기준이 "따라가기"에서 "골라내기"로 바뀐다 🟡 |
| **④ 도구 탐색** | 전향 수강생 | **무료 대체재와 만나는 지점.** 클로바노트·노션·다글로가 여기서 먼저 발견된다 🟢. 이 단계를 세우지 않으면 경쟁이 여정에서 사라진다 |
| **⑦ 수료·포트폴리오** | 전향 수강생 | 복습 기록이 **취업 자료로 전환되는 순간.** 확장 기능의 진입점이자 되돌릴 수 없는 지점(수료) |
| **① 자비 결제 결정 · ⑦ 환급 조건 정산** | 자비 수강생 | 지불 구조가 다르면 여정의 시작과 끝이 달라진다. **투자 회수 확인으로 끝난다** 🔴 |
| **② 녹음 금지 공지** | 녹음 금지반 수강생 | 여정이 여기서 끊긴다. **이 단계의 존재 자체가 발견** 🔴 |
| **② 자료 유출 가능성 검토** | 녹음을 금지한 강사 | 거부 결정이 만들어지는 단계. **여기 도달하기 전에 개입해야 한다** |
...
| **⑤ 우선순위 복습** | 태깅된 내용을 순위대로 복습, 남은 시간을 이력서에 배분 | "이만큼만 하면 되는 건가" | **통제감 회복** | 앱, 강사 태깅 승인, 리마인드 알림 | **PP-5** 우선순위를 신뢰할 근거가 필요하다 → **"이 판단은 이 강사(현직자)의 것"임을 표시** |
...
| **⑧ 면접·취업** | 지원·면접. 취업률 **54.2%** 🟢의 양쪽 어딘가 | "왜 이걸 중요하게 봤냐고 물으면" | 합격 시 **확신** / 불합격 시 **자기부정** | 채용 담당자, 기관 취업 지원 | **PP-8** 학습 근거를 말로 설명해야 한다 → **"현직자가 왜 이걸 중요하다고 했는지"가 그대로 면접 답변이 된다** |
...
|---|---|---|---|
| **전향 수강생** | ① 입과 | ①~⑧ 전 구간 | 주인공 |
| **현직자 강사** | **② (기관 도입 시 안내받고 진입)** | ②·③·⑤ | 태깅 판단을 공급. **①에서 거부하면 여정 자체가 없다** |
| **상위권 수강생** | ② (스스로 정리 시작) | ③·④에 비공식 관여 | 동기 질문에 답하며 **④의 도구 선택에 영향**을 준다 |
| **취업지원 담당자** | **⑦ 수료 시점** | ⑦·⑧ | 앞의 여섯 단계를 모른 채 결과물만 받는다. **확장 기능의 요구가 여기서 발생** |
| **알바 병행 수강생** | ① 입과 | ③에서 이탈 | 우선순위를 알아도 **복습할 시간이 0** → Extreme 분기 |

> **취업지원 담당자가 ⑦에서 처음 등장하는 것이 확장 기능의 구조적 문제입니다.** 포트폴리오 소재를 뽑으려면 ①~⑥의 복습 기록이 필요한데, **그 가치를 아는 사람은 ⑦에 와서야 나타납니다.** 데이터를 쌓아야 할 시점과 그것을 원하는 사람이 등장하는 시점이 어긋나 있습니다.
...
## 6. Extreme 여정 — 녹음 금지반 수강생

> 「녹음 금지라고 하셨다」 · **이 여정은 3단계에서 끊기고 입력이 발생하지 않습니다.**

| 단계 | 행동 | 생각·질문 | 감정 | Pain → 기회 |
|---|---|---|---|---|
| **① 과정 입과** | Core와 동일 | 동일 | 기대 | — |
| **② 녹음 금지 공지** | 강사·기관이 녹음 금지를 안내 🔴 | "그럼 이 앱은 못 쓰는 거네" | **박탈감** ← 최저점 | **PP-E1** 제품 작동의 **이진 조건**이 여기서 부정된다 → **강의 자료·판서 기반 대체 입력 경로**가 없으면 이 차수는 시장에서 빠진다 |
| **③ 필기 시도 → 강의 놓침** | 손으로 필기하며 따라가려다 강의를 놓친다 | "적다 보면 못 듣고, 들으면 못 적는다" | **좌절** | **PP-E2** 필기와 청취가 경합한다 → 강의 후 **자료 업로드 + 강사 태깅만으로 우선순위를 만드는 축소판** |
| — | **여정 종료. ④~⑧ 미도달** | | | |

> **이 여정이 시장 경계를 정합니다.** 녹음 가능 여부가 이진 조건이므로 **금지 비율이 곧 시장에서 빠지는 비율**입니다. [§9](#9-검증이-필요한-가정)의 검증 1순위입니다.
...
## 7. Non-user 여정 — 녹음을 금지한 강사

> 「내 강의가 밖으로 나간다」 · **도입에 도달하지 않지만 재진입 조건이 있습니다.**
...
| **④ 차수 전체 차단** | 해당 차수에서 제품 사용 불가 | — | — | **PP-N3** Core 전원이 찬성해도 이 사람이 막으면 끝난다 |
| **⑤ 재진입 조건** | 자산 귀속·삭제가 보장되면 재검토 | "내가 태깅 데이터의 주인이 된다면" | 조건부 개방 | **기회** — **강사를 데이터 소유자·공동 저자로 세우는 설계.** 거부권자를 진입장벽 공급자로 전환 |
...
**녹음 허용 여부는 개입 지점이 아니라 제품 성립 조건입니다.**

| 확인할 것 | 성립하지 않으면 |
|---|---|
| 강의 녹음이 허용되는 과정의 비율 | 금지 차수는 시장에서 빠진다. 대체 입력 경로 필요 |
| 강사가 태깅 판단을 제공할 의향 | **진입장벽이 사라지고 다글로와 구별되지 않는다** 🟢 |
...
|---|---|---|---|
| **1** | **강사 자산 귀속·삭제 보장을 도입 안내에 선제시** (PP-N1·PP-K5) | 강사 ② = 기관 운영자 ⑤ = 학습자 ② | **거부권이 만들어지는 순간에 개입한다.** 막히면 차수 전체가 사라지고, 반대로 강사를 데이터 소유자로 세우면 **거부권자가 진입장벽 공급자로 전환**된다 |
...
| 우선순위 | 항목 | 흔들리면 바뀌는 것 |
|---|---|---|
| **1** | **강의 녹음이 금지되는 과정의 비율** 🔴 | 제품 작동의 이진 조건. 시장 규모가 직접 줄어든다 |
| **2** | **강사가 태깅 판단을 제공할 의향** 🔴 | **진입장벽의 존재 여부.** 없으면 다글로와 구별되지 않는다([R] §6 1순위) |
