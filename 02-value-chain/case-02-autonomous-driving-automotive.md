...
graph LR
    subgraph SUP["지원활동"]
        I["기업 인프라 — 안전 거버넌스·인증·리콜 권한"]
        H["인적자원 — 차량공학 + SW 역량 결합"]
        T["기술개발 — 차량 OS·인지 모델·검증 툴체인"]
        P["조달 — 반도체·배터리·학습 컴퓨트"]
    end
    A1["투입·조달<br/>부품 + 주행 데이터"] --> A2["운영·생산<br/>차량 양산 + 모델 검증"]
    A2 --> A3["산출·배송<br/>차량 인도 + OTA"]
    A3 --> A4["마케팅·영업<br/>판매 + 구독 + 플릿"]
    A4 --> A5["서비스<br/>예측 정비 · OTA 리콜 · 사고 대응"]
    A5 -.->|"필드 데이터 재투입"| A1
    A5 --> M["마진"]
```
