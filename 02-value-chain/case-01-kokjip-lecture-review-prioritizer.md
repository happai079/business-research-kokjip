...
- **가치도 병목도 ②입니다.** 녹음·전사·요약은 무료 대체재가 이미 덮었으므로([1K] 2-4) 유료의 근거는 **판정 구간** 하나뿐이고, 같은 자리가 병목입니다.
- **그 병목은 두 겹입니다** — **품질**은 판정 근거의 신뢰도(제품의 존폐), **원가**는 사용량에 비례하는 추론 비용(마진율).
- **③은 배송이 아니라 두 개의 다른 설득입니다** — 학습자에게는 행동 유발, 기관에게는 성과 증빙. 후자가 다음 계약을 만들어 실질적으로 ④의 일부로 작동합니다.
- **순환 고리(⑤→①)가 유일한 축적 구조입니다** — 취업 결과 라벨이 강사 발화 신호의 가중치를 교정하며, **경쟁자가 살 수도 만들 수도 없는** 자산입니다.
- **지원활동 중 기업 인프라가 이례적으로 무겁습니다** — 원재료가 타인의 저작물이자 제3자의 음성이라 법적 설계가 **사업 가능 여부의 전제**입니다.
...
- **중복 수집 제거 — 단 재사용되는 것과 안 되는 것을 갈라야 합니다.** 판정 결과는 한 덩어리가 아니라 두 층입니다.

  | 층 | 내용 | 재사용 |
  |---|---|---|
  | **개념 중요도 맵** | "이 과정에서 A 개념은 실무 비중이 높다" — 커리큘럼 단위 | **가능** — KDT는 표준 커리큘럼을 공유하므로 다른 기관·다음 차수에 그대로 쓴다 |
  | **발화 정렬 층** | 그 개념이 **이 강의 몇 분에, 어떤 강조로** 나왔는가 | **불가** — 강사마다 발화가 다르므로 매번 새로 만든다 |

  **1차 신호가 강사 발화라는 선택이 재사용률의 상한을 정합니다.** 강사가 다르면 아래층은 다시 계산해야 하므로, 재사용은 원가를 0으로 만드는 것이 아니라 **위층만큼 깎습니다.** 이 상한이 곧 [§4](#4-마진-구조-진단)가 말하는 마진의 상한입니다.
...
**마진이 생기는 지점**
- **과정 단위 판정 재사용 — 단 위층에 한합니다.** KDT는 표준 커리큘럼을 공유하므로 **개념 중요도 맵**은 다른 기관·다음 차수에 그대로 재사용할 수 있습니다. **기관이 늘수록 한 건당 판정 원가가 떨어지는 유일한 구조**이지만, **발화 정렬 층은 강사마다 다시 만들어야 하므로 원가가 0으로 수렴하지는 않습니다**([§2-1](#2-1-투입조달-물류-inbound-logistics)).
- 전사·판정의 모델 등급 분리.
- 기관 라이선스(연 1,000만 원 🔴)는 개인 구독(연 6만 원 🔴)보다 계정당 획득 효율이 압도적으로 높습니다.
- 취업 결과 라벨의 축적 — 원가를 낮추지는 않지만 **경쟁자가 복제할 수 없는 자산**이라 가격 방어력을 만듭니다.

> **핵심 문장**: 이 사업의 수익성은 판정을 잘하는 정도가 아니라, **한 번 만든 판정 중 재사용되는 층이 얼마나 두꺼운가**로 결정됩니다. 커리큘럼이 표준화된 국비 교육을 표적으로 삼은 것은 시장 크기 때문만이 아니라 **위층(개념 중요도 맵)이 기관을 건너 재사용되기 때문**에도 옳은 선택입니다.
>
> **다만 이 강점은 판정 근거 선택과 맞물려 있습니다.** 1차 신호를 강사 발화로 정한 이상 아래층은 매번 새로 만들어야 하므로, **재사용률의 현실적 상한이 이 사업 마진의 상한**입니다. 그 상한을 실측하기 전까지 "기관이 늘수록 원가가 떨어진다"는 주장은 방향만 맞고 크기는 모릅니다 🔴.
...
- [ ] **과정 단위 판정 재사용률의 현실적 상한** — 같은 KDT 과정이라도 강사가 다르면 발화가 다르다. **§4 마진 구조 전체가 이 값 하나에 걸려 있다** 🔴
